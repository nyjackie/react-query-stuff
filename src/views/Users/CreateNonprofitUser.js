      ...
      Promise.all([
        ...
        if (isUniqueEmail && isUniquePhone) {
          postUser(values).then(() => {
            sendPasswordReset(values.email, values.sendEmail);
          });
        }
      });
    },
      ...
      <Row>
        <Col>
          ...
          <Form noValidate onSubmit={formik.handleSubmit}>
            ...
            {emailError && <p className="mt-2 text-danger">{emailError.message}</p>}
            {(ueLoading || upLoading || isLoading || emailLoading) && (
            ...
            {sendEmailSuccess && <p className="mt-2 text-success">Email Sent!</p>}
          </Form>
        </Col>
      </Row>
    </Container>
